...
class _CourierHomeState extends State<CourierHome>{
 ...
 ...vehicles.map((v)=>Card(child:ListTile(leading:const Icon(Icons.directions_car),title:Text("${v['registration']} • ${v['name']}"),trailing:Row(mainAxisSize:MainAxisSize.min,children:[if(selected?['id']==v['id'])const Icon(Icons.check_circle),IconButton(icon:const Icon(Icons.info_outline),onPressed:()=>Navigator.push(c,MaterialPageRoute(builder:(_)=>VehicleDetailsPage(vehicle:v))))]),onTap:()=>choose(v))))
 ])));
}
